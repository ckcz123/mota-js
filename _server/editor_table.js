editor_table_wrapper = function (editor) {
    ...
    class editor_table {
        ...
        checkboxSetMember(value, key, prefixString) {
        ...
        editGrid(showComment, type) {
            var list = [];
            if (showComment)
                list.push("<button onclick='editor.table.onCommentBtnClick(this)'>注释</button>");
            ...
            if (type == 'disable')
                list.push("<button onclick='editor.table.onCopyBtnClick(this)'>复制</button>");
            return list.join(' ');
        }
        title() {
        ...
        tr(guid, field, shortField, commentHTMLescape, cobjstr, shortComment, tdstr, type) {
        ...
        /**
             ...
             * 故放在HTML模板分类下
             */
        checkboxSetMemberOnchange(onemember) {
            ...
            for (var i in inputs) {
                if (inputs[i].nodeName == 'INPUT') {
                    if (inputs[i].checked) {
                        ...
                        if (inputs[i].getAttribute('ctype') == 'number')
                            one = parseFloat(one);
                        value.push(one);
                    }
                }
            ...
            var recursionParse = function (pfield, pcfield, pvobj, pcobj) {
                ...
                for (var ii in keysForTableOrder) {
                    ...
                    if (pcobj && pcobj['_data'] && pcobj['_data'][ii]) {
                        ...
                        if (pcobj && (pcobj['_data'] instanceof Function))
                            cobj = Object.assign({}, defaultcobj, pcobj['_data'](ii));
                        else
                            cobj = Object.assign({}, defaultcobj);
                    }
                    ...
                    for (var key in cobj) {
                        if (key === '_data')
                            continue;
                        if (cobj[key] instanceof Function)
                            cobj[key] = cobj[key](args);
                    }
         ...
         * @param {Object} cobj
         */
        objToTr(obj, commentObj, field, cfield, vobj, cobj) {
        ...
        objToTd(obj, commentObj, field, cfield, vobj, cobj) {
            var thiseval = vobj;
            switch (cobj._type) {
                case 'select':
        ...
        /**
             ...
             * @param {String} guid
             */
        guidListen(guid, tableid, obj, commentObj) {
            // tr>td[title=field]
            //   >td[title=comment,cobj=cobj:json]
            //   >td>div>input[value=thiseval]
        ...
        /**
             * 表格的值变化时
             */
        onchange(guid, obj, commentObj, thisTr, input, field, cobj, modeNode) {
            editor_mode.onmode(editor_mode._ids[modeNode.getAttribute('id')]);
            if (editor.mode.currentTable != thisTr.getAttribute('tableid'))
                return;
            var thiseval = null;
        ...
        onEditBtnClick(button) {
            ...
            if (cobj._type === 'event')
                editor_blockly.import(guid, { type: cobj._event });
            if (cobj._type === 'textarea')
                editor_multi.import(guid, { lint: cobj._lint, string: cobj._string });
            if (cobj._type === 'material')
                editor.table.selectMaterial(input, cobj);
            if (cobj._type === 'color')
                editor.table.selectColor(input);
            if (cobj._type === 'point')
                editor.table.selectPoint(input);
            if (cobj._type === 'popCheckboxSet')
                editor.table.popCheckboxSet(input, cobj);
        }
        onCopyBtnClick(button) {
            var tr = button.parentNode.parentNode;
            var input = tr.children[2].children[0].children[0];
         ...
         */
        dblclickfunc(guid, obj, commentObj, thisTr, input, field, cobj, modeNode) {
         ...
         * 删除表格项
         */
        deletefunc(guid, obj, commentObj, thisTr, input, field, cobj, modeNode) {
            editor_mode.onmode(editor_mode._ids[modeNode.getAttribute('id')]);
         ...
         * 添加表格项
         */
        addfunc(guid, obj, commentObj, thisTr, input, field, cobj, modeNode) {
            editor_mode.onmode(editor_mode._ids[modeNode.getAttribute('id')]);
