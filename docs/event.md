...
``` js
"x, y": [ // 实际执行的事件列表
  {"type": "changeFloor", "floorId": "sample0"，"loc": [10, 10], "direction": "left", "time": 1000 }, //后面几项依次为楼层id，楼层位置（不能放置在楼梯位置）两项为必填，勇士方向可选，切换时间也是可选。
]
...
``` js
"x, y": [ // 实际执行的事件列表
  {"type": "changePos", "loc": [10, 10], "direction": "left"}, // 直接切换勇士的坐标，loc为目标地点必填，后面勇士换位后方向可选
]
