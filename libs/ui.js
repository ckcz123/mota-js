...
ui.prototype._fillTextWithMaxWidth = function (ctx, text, x, y, maxWidth) {
    // 获得当前字体
    const font = ctx.font, u = /(\d+)px/.exec(font);
    if (u == null) return ctx.fillText(text, x, y);
    for (let font_size = parseInt(u[1]); font_size >= 8; font_size--) {
...
ui.prototype.fillPolygon = function (name, nodes, style) {
    ...
    for (let i = 0; i < nodes.length; ++i) {
        const x = core.calValue(nodes[i][0]), y = core.calValue(nodes[i][1]);
...
ui.prototype.splitLines = function (name, text, maxWidth, font) {
    ...
    for (let i = 0; i < text.length; i++) {
        if (text.charAt(i) === '\n') {
            ...
            const toAdd = text.substring(last, i + 1);
            const width = core.calWidth(name, toAdd);
...
ui.prototype.drawText = function (contents, callback) {
    ...
    const data = core.status.event.data.list.shift();
...
ui.prototype._getTitleAndIcon = function (content) {
    let title = null, image = null, icon = null, height = 32, animate = 1;
    content = content.replace(/(\t|\\t)\[(([^\],]+),)?([^\],]+)\]/g, function (s0, s1, s2, s3, s4) {
        if (s4) {
            if (s4 === 'hero') {
                ...
                icon = 0;
                const w = core.material.icons.hero.width || 32;
                ...
                const blockInfo = core.getBlockInfo(s4);
                if (blockInfo != null) {
...
ui.prototype._getPosition = function (content) {
    let pos = null, px = null, py = null, noPeak = false;
    ...
    content = content.replace("\b", "\\b")
        .replace(/\\b\[(up|center|down|hero|null)(,(hero|null|\d+,\d+|\d+))?]/g, function (s0, s1, s2, s3) {
            ...
            if (s3 === 'hero' || s1 === 'hero' && !s3) {
                ...
                px = py = null;
                if (str.length === 1) {
                    const follower = (core.status.hero.followers || [])[parseInt(str[0]) - 1];
                    if (follower) {
                        px = follower.x;
                        py = follower.y;
                    }
...
ui.prototype.drawWindowSelector = function (background, x, y, w, h) {
    ...
    const ctx = core.ui.createCanvas("_selector", x, y, w, h, 165);
    this._drawSelector(ctx, background, w, h);
}

ui.prototype._uievent_drawSelector = function (data) {
    const canvasName = '_uievent_selector_' + (data.code || 0);
    if (data.image == null) return core.deleteCanvas(canvasName);

    const background = data.image || core.status.textAttribute.background;
    if (typeof background != 'string') return;
    const x = core.calValue(data.x), y = core.calValue(data.y);
    let w = core.calValue(data.width), h = core.calValue(data.height);
    w = Math.round(w);
    h = Math.round(h);
    if (main.mode === 'editor') {
    ...
    const ctx = core.createCanvas(canvasName, x, y, w, h, z);
...
ui.prototype.drawWindowSkin = function (background, ctx, x, y, w, h, direction, px, py) {
    ...
    // 仿RM窗口皮肤 ↓
    const dstImage = core.getContextByName(ctx);
    if (!dstImage) return;
    // 绘制背景
...
ui.prototype.drawBackground = function (left, top, right, bottom, posInfo) {
    posInfo = posInfo || {};
    const px = posInfo.px == null || posInfo.noPeak ? null : posInfo.px * 32 - core.bigmap.offsetX;
    const py = posInfo.py == null || posInfo.noPeak ? null : posInfo.py * 32 - core.bigmap.offsetY;
    const xoffset = posInfo.xoffset || 0, yoffset = posInfo.yoffset || 0;
    let background = core.status.textAttribute.background;
...
ui.prototype._uievent_drawBackground = function (data) {
    this._createUIEvent();
    const background = data.background || core.status.textAttribute.background;
    const x = core.calValue(data.x), y = core.calValue(data.y), w = core.calValue(data.width),
...
ui.prototype._drawBackground_drawWindowSkin = function (background, left, top, right, bottom, position, px, py) {
    if (typeof background == 'string' && core.material.images.images[background]) {
        const image = core.material.images.images[background];
...
ui.prototype._calTextBoxWidth = function (ctx, content, min_width, max_width, font) {
    // 无限长度自动换行
    const allLines = core.splitLines(ctx, content, null, font);

    // 如果不存在手动换行，尽量调成半行形式
    if (allLines.length === 1) {
        const w = core.calWidth(ctx, allLines[0]) + 10;
...
ui.prototype._buildFont = function (fontSize, bold, italic) {
    const textAttribute = core.status.textAttribute || core.initStatus.textAttribute,
...
ui.prototype.drawTextContent = function (ctx, content, config) {
    ...
    // 创建一个新的临时画布
    const tempCtx = core.createCanvas('__temp__', 0, 0, ctx == null ? 1 : ctx.canvas.width, ctx == null ? 1 : ctx.canvas.height, -1);
...
ui.prototype._drawTextContent_draw = function (ctx, tempCtx, content, config) {
    ...
    const _drawNext = function () {
        ...
        const block = config.blocks[config.index++];
...
ui.prototype._drawTextContent_next = function (tempCtx, content, config) {
    ...
    // get next character
    const ch = content.charAt(config.index++);
...
ui.prototype._drawTextContent_drawChar = function (tempCtx, content, config, ch) {
    ...
    if (ch === '\\') {
        const c = content.charAt(config.index);
    ...
    // 检查是不是自动换行
    const charwidth = core.calWidth(tempCtx, ch) + config.interval;
    ...
    // 输出
    const left = config.offsetX, top = config.offsetY + config.topMargin;
...
ui.prototype._drawTextContent_newLine = function (tempCtx, config) {
    // 计算偏移量
    const width = config.offsetX, totalWidth = config.right - config.left;
    let marginLeft = 0;
...
ui.prototype._drawTextContent_changeColor = function (tempCtx, content, config) {
    ...
    if (content.charAt(index) === '[' && ((index2 = content.indexOf(']', index)) >= 0)) {
        // 变色
        const str = content.substring(index + 1, index2);
...
ui.prototype._drawTextContent_drawIcon = function (tempCtx, content, config) {
    ...
    if (content.charAt(config.index + 1) === '[' && ((index2 = content.indexOf(']', index + 1)) >= 0)) {
        const str = content.substring(index + 2, index2);
        // --- 获得图标
        const iconInfo = core.ui._getDrawableIconInfo(str), image = iconInfo[0], icon = iconInfo[1];
        if (image == null) return this._drawTextContent_next(tempCtx, content, config);
        // 检查自动换行
        const width = config.currfont + 2, left = config.offsetX + 2, top = config.offsetY + config.topMargin - 1;
...
ui.prototype.drawTextBox = function (content, showAll) {
    ...
    // Step 1: 获得标题信息和位置信息
    const textAttribute = core.status.textAttribute;
    const titleInfo = this._getTitleAndIcon(content);
    const posInfo = this._getPosition(titleInfo.content);
    ...
    // Step 2: 计算对话框的矩形位置
    const hPos = this._drawTextBox_getHorizontalPosition(content, titleInfo, posInfo);
    const vPos = this._drawTextBox_getVerticalPosition(content, titleInfo, posInfo, hPos.validWidth);

    // Step 3: 绘制背景图
    const pInfo = core.clone(posInfo);
    pInfo.xoffset = hPos.xoffset;
    pInfo.yoffset = vPos.yoffset - 4;
    const isWindowSkin = this.drawBackground(hPos.left, vPos.top, hPos.right, vPos.bottom, pInfo);
    const alpha = isWindowSkin ? this._drawWindowSkin_getOpacity() : textAttribute.background[3];

    // Step 4: 绘制标题、头像、动画
    const content_top = this._drawTextBox_drawTitleAndIcon(titleInfo, hPos, vPos, alpha);

    // Step 5: 绘制正文
    const config = this.drawTextContent('ui', content, {
        left: hPos.content_left,
        top: content_top,
        maxWidth: hPos.validWidth,
        lineHeight: vPos.lineHeight,
        time: (showAll || textAttribute.time <= 0 || core.status.event.id !== 'action') ? 0 : textAttribute.time
    });
    ...
    let left = (hPos.left + hPos.right) / 2;
...
ui.prototype._drawTextBox_drawImages = function (content) {
    return content.replace(/(\f|\\f)\[(.*?)]/g, function (text, sympol, str) {
        ...
        const img = core.material.images.images[ss[0]];
        if (!img) return "";
        // 绘制
        if (ss.length === 3)
...
ui.prototype._drawTextBox_getHorizontalPosition = function (content, titleInfo, posInfo) {
    const realContent = this._getRealContent(content);
    let paddingLeft = 25;
    ...
    let left = 7 + 3 * (this.HSIZE - 6), right = this.PIXEL - left,
        width = right - left, validWidth = width - paddingLeft - paddingRight;
    // 对话框效果：改为动态计算
    if (posInfo.px != null && posInfo.py != null) {
        let min_width = 220 - paddingLeft;
        const max_width = validWidth;
...
ui.prototype._drawTextBox_getVerticalPosition = function (content, titleInfo, posInfo, validWidth) {
    const textAttribute = core.status.textAttribute || core.initStatus.textAttribute;
    const lineHeight = textAttribute.textfont + 6;
    let height = 45 + this.getTextContentHeight(content, {
    ...
    const yoffset = 16;
    let top = parseInt((this.PIXEL - height) / 2);
    switch (posInfo.position) {
        case 'center':
            top = parseInt((this.PIXEL - height) / 2);
            break;
        case 'up':
            if (posInfo.px == null || posInfo.py == null)
...
ui.prototype._drawTextBox_drawTitleAndIcon = function (titleInfo, hPos, vPos, alpha) {
    ...
    const textAttribute = core.status.textAttribute;
    let content_top = vPos.top + 15;
    let image_top = vPos.top + 15;
    if (titleInfo.title != null) {
        const titlefont = textAttribute.titlefont;
        ...
        // --- title也要居中或者右对齐？
        const title_width = core.calWidth('ui', titleInfo.title, this._buildFont(titlefont, true));
        let title_left = hPos.content_left;
...
ui.prototype._createTextCanvas = function (content, lineHeight) {
    const width = this.PIXEL, height = 30 + this.getTextContentHeight(content, {lineHeight: lineHeight});
...
ui.prototype.drawScrollText = function (content, time, lineHeight, callback) {
    ...
    time = time || 5000;
    this.clearUI();
    const offset = core.status.textAttribute.offset || 15;
    lineHeight *= core.status.textAttribute.textfont;
    const ctx = this._createTextCanvas(content, lineHeight);
    const obj = {align: core.status.textAttribute.align, lineHeight: lineHeight};
...
ui.prototype._drawScrollText_animate = function (ctx, time, callback) {
    ...
    const per_pixel = 1, height = ctx.canvas.height, per_time = time * per_pixel / (this.PIXEL + height);
    const currH = this.PIXEL;
...
ui.prototype.drawChoices = function (content, choices) {
    ...
    const hPos = this._drawChoices_getHorizontalPosition(titleInfo, choices);
    const vPos = this._drawChoices_getVerticalPosition(titleInfo, choices, hPos);
    core.status.event.ui.offset = vPos.offset;

    const isWindowSkin = this.drawBackground(hPos.left, vPos.top, hPos.right, vPos.bottom);
...
ui.prototype._drawChoices_getHorizontalPosition = function (titleInfo, choices) {
    ...
    const left = (this.PIXEL - width) / 2, right = left + width;
    const content_left = left + (titleInfo.icon == null ? 15 : 60), validWidth = right - content_left - 10;
...
ui.prototype._drawChoices_getVerticalPosition = function (titleInfo, choices, hPos) {
    ...
    let height = 32 * (length + 2), bottom = this.HPIXEL + height / 2;
    if (length % 2 === 0) bottom += 16;
    let offset = 0;
    let choice_top = bottom - height + 56;
...
ui.prototype._drawChoices_drawTitle = function (titleInfo, hPos, vPos) {
    ...
    let content_top = vPos.top + 21;
    if (titleInfo.title != null) {
        core.setTextAlign('ui', 'center');

        content_top = vPos.top + 41;
        let title_offset = hPos.left + hPos.width / 2;
        // 动画
...
ui.prototype._drawChoices_drawChoices = function (choices, isWindowSkin, hPos, vPos) {
    ...
    core.setFont('ui', this._buildFont(17, true));
    for (let i = 0; i < choices.length; i++) {
        let color = choices[i].color || core.status.textAttribute.text;
        if (color instanceof Array) color = core.arrayToRGBA(color);
        core.setFillStyle('ui', color);
        let offset = this.HPIXEL;
        if (choices[i].icon) {
            const iconInfo = this._getDrawableIconInfo(choices[i].icon), image = iconInfo[0], icon = iconInfo[1];
            if (image != null) {
    ...
    if (choices.length > 0) {
        ...
        const len = choices[core.status.event.selection].width;
        if (isWindowSkin)
...
ui.prototype.drawConfirmBox = function (text, yesCallback, noCallback) {
    ...
    const rect = this._drawConfirmBox_getRect(contents);
    const isWindowSkin = this.drawBackground(rect.left, rect.top, rect.right, rect.bottom);
    ...
    const strokeLeft = this.HPIXEL + (76 * core.status.event.selection - 38) - parseInt(len / 2) - 5;
...
ui.prototype._drawConfirmBox_getRect = function (contents) {
    const max_width = contents.reduce(function (pre, curr) {
        return Math.max(pre, core.calWidth('ui', curr));
    }, 0);
    const left = Math.min(this.HPIXEL - 40 - parseInt(max_width / 2), 100), right = this.PIXEL - left;
    const top = this.HPIXEL - 68 - (contents.length - 1) * 30, bottom = this.HPIXEL + 68;
...
ui.prototype.drawWaiting = function (text) {
    ...
    core.clearUI();
    text = core.replaceText(text || "");
    const text_length = core.calWidth('ui', text, this._buildFont(19, true));
    const width = Math.max(text_length + 80, 220), left = this.HPIXEL - parseInt(width / 2), right = left + width;
    const top = this.HPIXEL - 48, height = 96, bottom = top + height;
...
ui.prototype.drawQuickShop = function () {
    ...
    const shopList = core.status.shops, keys = Object.keys(shopList).filter(function (shopId) {
        return shopList[shopId].visited || !shopList[shopId].mustEnable
    });
    const choices = keys.map(function (shopId) {
...
ui.prototype.drawPagination = function (page, totalPage, y) {
    ...
    const length = core.calWidth('ui', page + " / " + page, this._buildFont(15, true));
...
ui.prototype.drawCursor = function () {
    const automaticRoute = core.status.automaticRoute;
...
ui.prototype.drawBook = function (index) {
    const floorId = core.floorIds[(core.status.event.ui || {}).index] || core.status.floorId;
    let enemys = core.enemys.getCurrentEnemys(floorId);
    core.clearUI();
    core.clearMap('data');
    // 生成groundPattern
    ...
    const pageinfo = this._drawBook_pageinfo();
    const perpage = pageinfo.per_page, page = parseInt(index / perpage) + 1,
...
ui.prototype._drawBook_pageinfo = function () {
    const per_page = this.HSIZE; // 每页个数
    const padding_top = 12; // 距离顶端像素
    const per_height = (this.PIXEL - 32 - padding_top) / per_page;
...
ui.prototype._drawBook_drawOne = function (floorId, index, enemy, pageinfo, selected) {
    ...
    const top = pageinfo.per_height * index + pageinfo.padding_top; // 最上面margin默认是12px
    // 横向规划：
    // 22 + 42 = 64 是头像框
    this._drawBook_drawBox(index, enemy, top, pageinfo);
    // 剩余 PIXEL - 64 的宽度，按照 10 : 9 : 8 : 8 的比例划分
    const left = 64, total_width = this.PIXEL - left;
    const name_width = total_width * 10 / 35;
...
ui.prototype._drawBook_drawBox = function (index, enemy, top, pageinfo) {
    ...
    const border_top = top + (pageinfo.per_height - 42) / 2, border_left = 22;
    const img_top = border_top + 5, img_left = border_left + 5;
    core.strokeRect('ui', 22, border_top, 42, 42, '#DDDDDD', 2);
    const blockInfo = core.getBlockInfo(enemy.id);
    core.status.boxAnimateObjs.push({
...
ui.prototype._drawBook_drawContent = function (index, enemy, top, left) {
    const width = this.PIXEL - left; // 9 : 8 : 8 划分三列
...
ui.prototype._drawBook_drawRow1 = function (index, enemy, top, left, width, position) {
    // 绘制第一行
    core.setTextAlign('ui', 'left');
    const b13 = this._buildFont(13, true), f13 = this._buildFont(13, false);
    const col1 = left, col2 = left + width * 9 / 25, col3 = left + width * 17 / 25;
...
ui.prototype._drawBook_drawRow2 = function (index, enemy, top, left, width, position) {
    ...
    let damage_offset = col1 + (this.PIXEL - col1) / 2 - 12;
    // 第一列
...
ui.prototype._drawBook_drawDamage = function (index, enemy, offset, position) {
    core.setTextAlign('ui', 'center');
    let damage = enemy.damage, color = '#FFFF00';
    if (damage == null) {
        damage = '无法战斗';
        color = '#FF0000';
    } else {
...
ui.prototype.drawBookDetail = function (index) {
    const info = this._drawBookDetail_getInfo(index), enemy = info[0];
    if (!enemy) return;
    const content = info[1].join("\n");
    core.status.event.id = 'book-detail';
    core.clearTip();
    core.clearMap('data');

    const left = 10, width = this.PIXEL - 2 * left, right = left + width;
    const content_left = left + 25, validWidth = right - content_left - 13;
    const contents = core.splitLines("data", content, validWidth, this._buildFont(16, false));
    const height = Math.max(24 * contents.length + 55, 80), top = (this.PIXEL - height) / 2, bottom = top + height;
...
ui.prototype._drawBookDetail_getInfo = function (index) {
    ...
    index = core.clamp(index, 0, enemys.length - 1);
    const enemy = enemys[index], enemyId = enemy.id;
    const texts = core.enemys.getSpecialHint(enemyId);
...
ui.prototype._drawBookDetail_mofang = function (enemy, texts) {
    ...
    if (core.enemys.hasSpecial(enemy.special, 10)) {
        const hp = enemy.hp;
        const delta = core.status.hero.atk - core.status.hero.def;
...
ui.prototype._drawBookDetail_vampire = function (enemy, texts) {
    if (core.enemys.hasSpecial(enemy.special, 11)) {
        const damage = core.getDamage(enemy.id);
        if (damage != null) {
            // 二分HP
            let start = 1, end = 100 * damage;
            const nowHp = core.status.hero.hp;
            while (start < end) {
...
ui.prototype._drawBookDetail_turnAndCriticals = function (enemy, floorId, texts) {
    let damageInfo = core.getDamageInfo(enemy, null, null, null, floorId);
    texts.push("战斗回合数：" + ((damageInfo || {}).turn || 0));
    // 临界表
    const criticals = core.enemys.nextCriticals(enemy, 8, null, null, floorId).map(function (v) {
        return core.formatBigNumber(v[0]) + ":" + core.formatBigNumber(v[1]);
    });
    while (criticals[0] === '0:0') criticals.shift();
    texts.push("临界表：" + JSON.stringify(criticals));
    let prevInfo = core.getDamageInfo(enemy, {atk: core.status.hero.atk - 1}, null, null, floorId);
...
ui.prototype.drawFly = function (page) {
    ...
    const floorId = core.floorIds[page];
    ...
    const middle = this.HPIXEL + 39;

    // 换行
    const lines = core.splitLines('ui', title, 120, this._buildFont(19, true));
...
ui.prototype.drawCenterFly = function () {
    ...
    const toX = core.bigmap.width - 1 - core.getHeroLoc('x'), toY = core.bigmap.height - 1 - core.getHeroLoc('y');
    ...
    const offsetX = core.clamp(toX - core.__HALF_SIZE__, 0, core.bigmap.width - core.__SIZE__),
...
ui.prototype.drawShop = function (shopId) {
    ...
    let actions = [];
    const fromList = (core.status.event.data || {}).fromList, selection = core.status.event.selection;
    ...
    const times = shop.times, need = core.calValue(shop.need, null, null, times);
    const content = "\t[" + shop.name + "," + shop.icon + "]" + core.replaceText(shop.text, need, times);
    const use = shop.use === 'experience' ? '经验' : '金币';
    const choices = [];
    for (let i = 0; i < shop.choices.length; i++) {
        const choice = shop.choices[i];
        let text = core.replaceText(choice.text, need, times);
        if (choice.need != null)
...
ui.prototype.drawMaps = function (index, x, y) {
    ...
    const data = this._drawMaps_buildData(index, x, y);
    core.drawThumbnail(data.floorId, null, {damage: data.damage},
        {ctx: 'ui', centerX: data.x, centerY: data.y, all: data.all});
    // 绘图
    if (data.paint) {
        const offsetX = 32 * (data.x - this.HSIZE), offsetY = 32 * (data.y - this.HSIZE);
        let value = core.paint[data.floorId];
    ...
    let text = core.status.maps[data.floorId].title;
    if (!data.all && (data.mw > this.SIZE || data.mh > this.SIZE))
        text += " [" + (data.x - this.HSIZE) + "," + (data.y - this.HSIZE) + "]";
    const textX = 16, textY = 18, width = textX + core.calWidth('data', text) + 16, height = 42;
...
ui.prototype._drawMaps_drawHint = function () {
    ...
    core.setTextAlign('ui', 'center');
    const stroke = function (left, top, width, height, fillStyle, lineWidth) {
        core.strokeRect('ui', left * 32 + 2, top * 32 + 2, width * 32 - 4, height * 32 - 4, fillStyle, lineWidth);
    };
    const per = this.HSIZE - 4;
    ...
    const lt = ["左", "移", "地", "图", "[A]"], rt = ["右", "移", "地", "图", "[D]"];
    for (let i = 0; i < 5; ++i) {
...
ui.prototype._drawMaps_buildData = function (index, x, y) {
    let damage = (core.status.event.data || {}).damage;
    let paint = (core.status.event.data || {}).paint;
    let all = (core.status.event.data || {all: true}).all;
    ...
    const floorId = core.floorIds[index], mw = core.floors[floorId].width, mh = core.floors[floorId].height;
...
ui.prototype.drawToolbox = function (index) {
    const info = this._drawToolbox_getInfo(index);
    ...
    const line1 = this.PIXEL - 306;
    this._drawToolbox_drawLine(line1, "消耗道具");
    const line2 = this.PIXEL - 146;
...
ui.prototype._drawToolbox_getInfo = function (index) {
    ...
    // 获取物品列表
    const tools = Object.keys(core.status.hero.items.tools).sort();
    const constants = Object.keys(core.status.hero.items.constants).sort();
    // 处理页数
    const toolsPage = core.status.event.data.toolsPage;
    const constantsPage = core.status.event.data.constantsPage;
    const toolsTotalPage = Math.ceil(tools.length / this.LAST);
    const constantsTotalPage = Math.ceil(constants.length / this.LAST);
    // 处理index
    if (index == null)
...
ui.prototype._drawToolbox_drawDescription = function (info, max_height) {
    ...
    const item = core.material.items[info.selectId];
    core.fillText('ui', item.name, 10, 32, '#FFD700', this._buildFont(20, true))
    let text = item.text || "该道具暂无描述。";
    try {
        // 检查能否eval
        text = core.replaceText(text);
    } catch (e) {
    ...
    for (let font_size = 17; font_size >= 14; font_size -= 3) {
...
ui.prototype._drawToolbox_drawContent = function (info, line, items, page, drawCount) {
    ...
    for (let i = 0; i < this.LAST; i++) {
        const item = items[this.LAST * (page - 1) + i];
        if (!item) continue;
        const yoffset = line + 54 * Math.floor(i / this.HSIZE) + 19;
...
ui.prototype.drawEquipbox = function (index) {
    const info = this._drawEquipbox_getInfo(index);
    this._drawToolbox_drawBackground();
...
ui.prototype._drawEquipbox_getInfo = function (index) {
    ...
    const allEquips = core.status.globalAttribute.equipName;
    const equipLength = allEquips.length;
    if (!core.status.hero.equipment) core.status.hero.equipment = [];
    const equipEquipment = core.status.hero.equipment;
    const ownEquipment = Object.keys(core.status.hero.items.equips).sort();
    const page = core.status.event.data.page;
    const totalPage = Math.ceil(ownEquipment.length / this.LAST);
    // 处理index
    if (index == null) {
...
ui.prototype._drawEquipbox_description = function (info, max_height) {
    core.setTextAlign('ui', 'left');
    if (!info.selectId) return;
    const equip = core.material.items[info.selectId];
    // --- 标题
    if (!equip.equip) equip.equip = {"type": 0};
    let equipType = equip.equip.type, equipString;
    ...
    // --- 描述
    let text = equip.text || "该装备暂无描述。";
    try {
        text = core.replaceText(text);
    } catch (e) {
    }

    for (let font_size = 17; font_size >= 11; font_size -= 3) {
        var lines = core.splitLines('ui', text, this.PIXEL - 15, this._buildFont(font_size, false));
        var line_height = parseInt(font_size * 1.4), curr = 37 + line_height;
        if (curr + lines.length * line_height < max_height) break;
    }
...
ui.prototype._drawEquipbox_getStatusChanged = function (info, equip, equipType, y) {
    let compare, differentMode = null;
    if (info.index < this.LAST) compare = core.compareEquipment(null, info.selectId);
    else {
        if (equipType < 0) differentMode = '<当前没有该装备的空位，请先卸下装备>';
        else {
            const last = core.material.items[info.equipEquipment[equipType]] || {};
            if (last.equip && last.equip.percentage !== equip.equip.percentage)
...
ui.prototype._drawEquipbox_drawStatusChanged = function (info, y, equip, equipType) {
    const compare = this._drawEquipbox_getStatusChanged(info, equip, equipType, y);
    if (compare == null) return;
    const obj = {drawOffset: 10, y: y};

    // --- 变化值...
    core.setFont('ui', this._buildFont(14, true));
    for (const name in compare) {
        const img = core.statusBar.icons[name];
        const text = core.getStatusName(name);
        ...
        let nowValue = core.getStatus(name) * core.getBuff(name),
            newValue = (core.getStatus(name) + compare[name]) * core.getBuff(name);
        if (equip.equip.percentage) {
            const nowBuff = core.getBuff(name), newBuff = nowBuff + compare[name] / 100;
...
ui.prototype._drawEquipbox_drawStatusChanged_draw = function (text, color, obj) {
    const len = core.calWidth('ui', text);
...
ui.prototype._drawEquipbox_drawEquiped = function (info, line) {
    core.setTextAlign('ui', 'center');
    const per_line = this.HSIZE - 3, width = Math.floor(this.PIXEL / (per_line + 0.25));
    // 当前装备
    for (let i = 0; i < info.equipLength; i++) {
        const equipId = info.equipEquipment[i] || null;
        // var offset_text = width * (i % per_line) + 56;
        const offset_image = width * (i % per_line) + width * 2 / 3;
        const offset_text = offset_image - (width - 32) / 2;
        const y = line + 54 * Math.floor(i / per_line) + 19;
        if (equipId) {
            const icon = core.material.icons.items[equipId];
...
ui.prototype.drawSLPanel = function (index, refresh) {
    ...
    let page = parseInt(index / 10), offset = index % 10;
    let max_page = main.savePages || 30;
...
ui.prototype._drawSLPanel_drawRecord = function (title, data, x, y, size, cho, highLight) {
    ...
    if (data && data.floorId) {
        ...
        let v = core.formatBigNumber(data.hero.hp, true) + "/" + core.formatBigNumber(data.hero.atk, true) + "/" + core.formatBigNumber(data.hero.def, true);
        const v2 = "/" + core.formatBigNumber(data.hero.mdef, true);
...
ui.prototype._drawSLPanel_drawRecords = function (n) {
    const page = core.status.event.data.page;
    const offset = core.status.event.data.offset;
    const u = Math.floor(this.PIXEL / 6), size = Math.floor(this.PIXEL / 3 - 20);
    const name = core.status.event.id === 'save' ? "存档" : core.status.event.id === 'load' ? "读档" : "回放";

    for (let i = 0; i < (n || 6); i++) {
        const data = core.status.event.ui[i];
        const id = 5 * page + i;
        const highLight = (i > 0 && core.saves.favorite.indexOf(id) >= 0) || core.status.event.data.mode === 'fav';
        let title = (highLight ? '★ ' : '☆ ') + (core.saves.favoriteName[id] || (name + id));
        if (i !== 0 && core.status.event.data.mode === 'fav') {
            if (!data) break;
            const real_id = core.saves.favorite[id - 1];
            title = (core.saves.favoriteName[real_id] || (name + real_id)) + ' ✐';
        }

        const charSize = 32;// 字体占用像素范围
        const topSpan = parseInt((this.PIXEL - charSize - 2 * (charSize * 2 + size)) / 3);// Margin
        const yTop1 = topSpan + parseInt(charSize / 2) + 8;//文字的中心
        const yTop2 = yTop1 + charSize * 2 + size + topSpan;
        if (i < 3) {
...
ui.prototype.drawKeyBoard = function () {
    ...
    const width = 384, height = 320;
    const left = (this.PIXEL - width) / 2, right = left + width;
    const top = (this.PIXEL - height) / 2, bottom = top + height;

    const isWindowSkin = this.drawBackground(left, top, right, bottom);
    core.setTextAlign('ui', 'center');
...
ui.prototype.drawStatistics = function (floorIds) {
    const obj = this._drawStatistics_buildObj();
...
ui.prototype._drawStatistics_buildObj = function () {
    ...
    const ori = this.uidata.drawStatistics();
    const ids = ori.filter(function (e) {
    ...
    const order = ["doors", "keys", "items", "tools", "constants", "equips"];
    ids.sort(function (a, b) {
        const c1 = order.indexOf(cls[a]), c2 = order.indexOf(cls[b]);
...
ui.prototype._drawStatistics_floorId = function (floorId, obj) {
    const floor = core.status.maps[floorId], blocks = floor.blocks;
    // 隐藏层不给看
...
ui.prototype._drawStatistics_items = function (floorId, floor, id, obj) {
    let hp = 0, atk = 0, def = 0, mdef = 0;
    if (obj.cls[id] === 'items' && id !== 'superPotion') {
        const temp = core.clone(core.status.hero);
        core.setFlag("__statistics__", true);
        const ratio = floor.item_ratio || 1;
        try {
        ...
        const values = core.material.items[id].equip || {};
        atk = values.atk || 0;
        def = values.def || 0;
        mdef = values.mdef || 0;
    }
...
ui.prototype._drawStatistics_getName = function (key) {
    return {
        "yellowDoor": "黄门", "blueDoor": "蓝门", "redDoor": "红门", "greenDoor": "绿门",
        "steelDoor": "铁门"
    }[key] || core.material.items[key].name;
}
...
ui.prototype._drawPaint_draw = function () {
    ...
    // 将已有的内容绘制到route上
    let value = core.paint[core.status.floorId];
    if (value) value = lzw_decode(value).split(",");
...
ui.prototype.relocateCanvas = function (name, x, y) {
    const ctx = core.getContextByName(name);
    if (!ctx) return null;
    if (x != null) {
